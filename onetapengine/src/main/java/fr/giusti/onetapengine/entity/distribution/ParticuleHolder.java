...
public class ParticuleHolder {
    /**
     * Hold recycled particules
     * key= particule name (generated by type and version)
     */
     ...
     * Ressource intensive
     *
     * @param groupId          id for the type of desired particule
     * @param spawnArea        area in which the particules will start
     * @param generalDirection general direction in which the particul will move (is slightly altered for each paricule)
     * @param particulNb       nb of particules
     * @return A group of particule corresponding to given critters
     */
    public static ArrayList<Particule> getAvailableParticuleGroupe(String groupId, RectF spawnArea, PointF generalDirection, int particulNb) {
        ArrayList<Particule> result = new ArrayList<>(particulNb);
        ...
        for (int i = 0; i < particulNb; i++) {
            String particuleId = particuleIdList.get((int) (random.nextFloat() * particuleIdList.size()));
            ...
            int xMovement = (random.nextBoolean() ? 1 : -1) * (random.nextInt(3) + 1);
            int yMovement = (random.nextBoolean() ? 1 : -1) * (random.nextInt(3) + 1);
